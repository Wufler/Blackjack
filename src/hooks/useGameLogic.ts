...
function shuffleDeck(deck: PlayingCard[]) {
  return [...deck].sort(() => Math.random() - 0.5);
}

export function useGameLogic(
  playCardSound: () => void,
  playMixingSound: () => void,
) {
  ...
  const initializeDeck = useCallback(() => {
    ...
    const newDeck = suits.flatMap((suit) =>
      values.map((value) => ({ suit, value, hidden: false })),
    );
  ...
  const calculateHandValue = useCallback(
    (hand: PlayingCard[], includeHidden: boolean = false): HandValue => {
      let baseValue = 0;
      let numAces = 0;
  ...
  const checkInitialBlackjack = useCallback(
    (pHand: PlayingCard[], dHand: PlayingCard[]) => {
